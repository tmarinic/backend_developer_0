...
-- spoji tablice posudba sa filmovi i mediji preko vezne tablice posudba_kopija
...
SELECT f.*, COUNT(k.id) AS 'Ukupna Količina'
    FROM filmovi f
    JOIN kopija k ON k.film_id = f.id
    GROUP BY f.id;

-- HAVING - filtrirajte gornji upit da izlista samo filmove koji imaju vise od 5 kopija (promijenjen upit)
SELECT f.*, COUNT(k.id) AS ukupna_količina
    FROM filmovi f
    JOIN kopija k ON k.film_id = f.id
    GROUP BY f.id
    HAVING ukupna_količina > 5;

-- Subquery (Podupit)
-- Dohvati sve filmove koji imaju kolicinu na BlueRay-u
SELECT naslov
    FROM filmovi.ime from filmovi
    WHERE id IN (1,3,4);
    SELECT film_id
    FROM kopija k
    JOIN mediji m ON k.medij_id = m.id
